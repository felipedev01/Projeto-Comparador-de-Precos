...
function HomeScreen({ navigation }) {
  ...
  const [firebase, setarfirebase] = useState('')
  useEffect(()=>{
  ...
  const [filtered, setfiltered] = useState([{firebase}])
  ...
  const set = function (text) {
    ...
    if (text) {
      ...
      //alert(list)
      const list2 = firebase.filter(function (val) {
